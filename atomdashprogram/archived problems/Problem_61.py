...
from fpack import if_heptagonal_number, if_hexagonal_number, if_triangle_number, if_pentagonal_number, if_octagonal_number
                ...
                if str(c)[:2] == str(b)[-2:] and c!= b and c!= a:
                    for d in megset:
                        if str(d)[:2] == str(c)[-2:] and d!= c and d!= b and d != a:
                            for e in megset:
                                if str(e)[:2] == str(d)[-2:] and e!= d and e != c and e != b and e != a:
                                    for f in megset:
                                        if str(f)[:2] == str(e)[-2:] and f!= e and f != d and f != c and f != b and f != a and str(a)[:2] == str(f)[-2:]:
                                            loc = [a,b,c,d,e,f]
                                            if checkpurity(loc):
                                                print(checkpurity(loc), loc,  sum(loc), time() - t)
