...
for column in range(len(usable)-2, -1, -1):
    ...
    for rth in range(1,len(usable)):
        upper_layer[rth] = min(upper_layer[rth-1] + usable[rth][column], zero_matrix[rth][column+1] + usable[rth][column])

    zero_matrix[len(usable)-1][column] = upper_layer[len(usable)-1]

    for rev_rth in range(len(usable)-2,-1,-1):
        zero_matrix[rev_rth][column] = min(upper_layer[rev_rth], usable[rev_rth][column] + zero_matrix[rev_rth+1][column])
