...
with open(
    r"D:\utkarsh\Github_Directories\Repo_Archived_Problems\ArchiveddashProblems\read_from_here\p067_triangle.txt",
    "r",
) as file:
...
for q in range(0, len(the_answerful) - 1):
    for p in range(0, len(the_answerful[q]) - 1):
        if (
            the_answerful[q][p] + the_answerful[q + 1][p]
            > the_answerful[q][p + 1] + the_answerful[q + 1][p]
        ):
