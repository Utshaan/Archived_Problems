...
def cool(a: int, b: int) -> bool:
    return divide_primecheck(int(str(a) + str(b))) and divide_primecheck(
        int(str(b) + str(a))
    )
            ...
            for c in range(b + 1, len(primeset)):
                ...
                if cool(primeb, primec) and cool(primea, primec):
                    for d in range(c + 1, len(primeset)):
                        ...
                        if (
                            cool(primec, primed)
                            and cool(primeb, primed)
                            and cool(primea, primed)
                        ):
                            for e in range(d + 1, len(primeset)):
                                primee = primeset[e]
                                if (
                                    cool(primed, primee)
                                    and cool(primec, primee)
                                    and cool(primeb, primee)
                                    and cool(primea, primee)
                                ):
                                    print(
