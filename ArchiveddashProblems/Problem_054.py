...
def winner(A_cards, B_cards):
    ...
    if royal_flush(A_cards) or royal_flush(B_cards):
        ...
        if tiebreaker(A_cards, 2) == tiebreaker(B_cards, 2):
            return moretiebreaker(
                A_cards, tiebreaker(A_cards, 2), B_cards, tiebreaker(B_cards, 2)
            )
        else:
...
t = time()
with open(
    r"D:\utkarsh\Github_Directories\Repo_Archived_Problems\ArchiveddashProblems\read_from_here\p054_poker.txt",
    "r",
) as f:
    stuff = f.readlines()
    ans = []
